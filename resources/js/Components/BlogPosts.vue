<template>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 md:gap-10 px-3 sm:px-4 max-w-6xl mx-auto mt-6 sm:mt-8 md:mt-10">
        <div v-for="(item, index) in mediaItems" :key="index" 
            ...
            <div class="p-4 sm:p-6 md:p-8 flex flex-col flex-grow">
                <h2 class="text-lg sm:text-xl md:text-2xl font-semibold mb-2 text-gray-800">{{ item.title }}</h2>
                <div class="border-t border-b border-gray-200 my-3 sm:my-4 py-3 sm:py-4">
                ...
                <div class="mt-auto">
                    <div class="flex items-center justify-between">
                        <span class="text-xs sm:text-sm text-gray-500">{{ formatDate(item.created_at) }}</span>
                        <div class="flex items-center space-x-2 sm:space-x-3">
                            <!-- Facebook Share -->
                            <a :href="getFacebookShareUrl(item)" 
                               target="_blank"
                               class="text-blue-600 hover:text-blue-800 transition-colors p-1 sm:p-0"
                            ...
                            <a :href="getTwitterShareUrl(item)"
                               target="_blank"
                               class="text-sky-500 hover:text-sky-700 transition-colors p-1 sm:p-0"
                            ...
                            <a :href="getInstagramShareUrl(item)"
                               target="_blank"
                               class="text-pink-600 hover:text-pink-800 transition-colors p-1 sm:p-0"
                            ...
                            <button @click="copyLink(item)"
                                    class="text-gray-600 hover:text-gray-800 transition-colors p-1 sm:p-0"
                                    title="Copy link">
    ...
    <div v-if="showCopyToast" 
         class="fixed bottom-4 right-4 bg-green-500 text-white px-3 sm:px-4 py-2 text-sm sm:text-base rounded-lg shadow-lg transition-opacity duration-300 z-50"
        ...
        <div class="relative w-full max-w-xs sm:max-w-sm md:max-w-lg lg:max-w-2xl xl:max-w-4xl">
            ...
            <div class="relative group">
                ...
                <div v-if="selectedCaption"
                     class="absolute inset-0 bg-black bg-opacity-70 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out flex items-center justify-center p-4 sm:p-6 md:p-8 rounded-lg">
                    <p class="text-white font-serif italic text-center overflow-y-auto max-h-full text-sm sm:text-base md:text-lg" style="font-family: 'Cormorant Garamond', serif; text-shadow: 0 0 10px rgba(255,255,255,0.3);">
                        {{ selectedCaption }}
                    </p>
                </div>
            </div>
            ...
            <button
                @click="prevImage"
                class="absolute left-2 sm:left-3 md:left-5 lg:left-7 top-1/2 transform -translate-y-1/2 text-white text-lg sm:text-xl md:text-2xl lg:text-3xl bg-gray-700 bg-opacity-50 p-1 sm:p-1.5 md:p-2 rounded-full hover:bg-opacity-75 transition-colors duration-300"
                :class="{ 'opacity-50 cursor-not-allowed': currentImageIndex <= 0 && currentItemIndex <= 0 }">
                &#10094;
            </button>
            <button
                @click="nextImage"
                class="absolute right-2 sm:right-3 md:right-5 lg:right-7 top-1/2 transform -translate-y-1/2 text-white text-lg sm:text-xl md:text-2xl lg:text-3xl bg-gray-700 bg-opacity-50 p-1 sm:p-1.5 md:p-2 rounded-full hover:bg-opacity-75 transition-colors duration-300"
